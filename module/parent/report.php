...
$temp = mysqli_query($con,"SELECT * FROM parents WHERE id ='$check'");
$tem = mysqli_fetch_array($temp);

 $st=mysqli_query($con,"SELECT *  FROM students WHERE id='{$tem['studentid']}' ");
	...
	<div style="float: left; background-color: gray ">
		...
		<table border="1">
			...
			<tr>
				...
				$temp = mysqli_query($con,"SELECT * FROM parents WHERE id ='$check'");
				$tem = mysqli_fetch_array($temp);
				$attendmon = "SELECT DISTINCT(date) FROM attendance WHERE attendedid='{$tem['studentid']}'";
		...
		<table border="1">
			...
			<tr>
				...
				$tem = mysqli_fetch_array($temp);
					...
					$attendmon = "SELECT DISTINCT (date) FROM attendance WHERE  date not in (select DISTINCT(date) from attendance where attendedid='{$tem['studentid']}' )";
					$resmon = mysqli_query($con,$attendmon);
					echo "<tr> <th></th></tr>";
					while($r=mysqli_fetch_array($resmon))
					{
				...
				?></td>
								...
								<table border="" style="width: 100px">
									...
									$sql="SELECT * FROM grade WHERE studentid='{$tem['studentid']}'";
									...
									if($x['marks']<=100 && $x['marks']>=70){
										$grade = 'A';
										$up =mysqli_query($con,"UPDATE grade SET grade='$grade' WHERE  studentid = '{$tem['studentid']}' AND id= '{$x['id']}'");
								echo "
									<tr>
										<td>" ,$x['courseid'],"</td>
										<td>" ,$x['marks'],  "</td>
								 ...
								 ";
								  ...
								  $tot = mysqli_query($con,"SELECT SUM(marks) AS total FROM grade WHERE studentid = '{$tem['studentid']}'");
								  while($total = mysqli_fetch_array($tot)){
								 
								  

								  
								  $ave = mysqli_query($con,"SELECT COUNT(marks) AS average FROM grade WHERE  studentid = '{$tem['studentid']}'");
								 ...
								 <div class="container" style="background-color: white">
								 	<div align="center" style="color: ; th{color:green;}">
								 		...
								 		<table border="1">
								 			...
								 			<tr><td>
								 		<?php 
								 		$bal = mysqli_query($con,"SELECT * FROM payment WHERE parentid='$check'");
								 		
								 		$tot = mysqli_query($con,"SELECT SUM(`paid`) AS totbal FROM payment WHERE parentid='$check'");
								 		...
								 		$upt =mysqli_query($con,"UPDATE payment SET balance='$balance' WHERE parentid='$check'");
								 		...
								 		$tot = mysqli_query($con,"SELECT * FROM payment WHERE parentid='$check'");
								 		while($total = mysqli_fetch_array($tot)){
